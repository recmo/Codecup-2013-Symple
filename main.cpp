...
{
	const uint64 field = 0x7FFF7FFF7FFF7FFFULL;
	BoardMask result;
...
{
	int result = 0;
	result += __builtin_popcountll(bits[0]);
	result += __builtin_popcountll(bits[1]);
	result += __builtin_popcountll(bits[2]);
	result += __builtin_popcountll(bits[3]);
	return result;
}
...
{
	if(bits[0])
		return planePoint(0, __builtin_ctzll(bits[0]));
	if(bits[1])
		return planePoint(1, __builtin_ctzll(bits[1]));
	if(bits[2])
		return planePoint(2, __builtin_ctzll(bits[2]));
	if(bits[3])
		return planePoint(3, __builtin_ctzll(bits[3]));
	return BoardPoint();
}
...
BoardPoint BoardMask::randomPoint() const
{
	...
	int planeCount = __builtin_popcountll(bits[0]);
	if(index < planeCount)
		return planePoint(0, indexOfNthBit(bits[0], index));
	index -= planeCount;
	planeCount = __builtin_popcountll(bits[1]);
	if(index < planeCount)
		return planePoint(1, indexOfNthBit(bits[1], index));
	index -= planeCount;
	planeCount = __builtin_popcountll(bits[2]);
	if(index < planeCount)
...
BoardPoint BoardMask::planePoint(int plane, int index)
{
	...
	return BoardPoint(horizontal, 4 * plane + vertical);
}
